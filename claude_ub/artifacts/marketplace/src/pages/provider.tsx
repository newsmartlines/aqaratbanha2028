...
import { api, type ProviderDetail, type Provider, type Review } from "@/lib/api";
import { useAuth } from "@/lib/auth-context";

const DEFAULT_IMG =
  "https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=800&q=80";

function imgSrc(url: string | null | undefined, fallback = DEFAULT_IMG): string {
...
export default function ProviderPage({ params }: { params: { id: string } }) {
  ...
  const shareTitle = provider ? `${provider.userName} – دليل بلس` : "دليل بلس";
  ...
  const isFavorite = favorites?.some((f) => f.providerId === providerId) ?? false;
  ...
  /* ─── Request service ─── */
  const requestService = (serviceId?: number) => {
    if (!user) { setLocation("/login"); return; }
    api.requests.create({ providerId, serviceId, userId: user.id });
  };
  ...
  const bannerSrc = imgSrc(provider.banner ?? provider.avatar);
  const avatarSrc = imgSrc(provider.avatar);
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col font-sans bg-background" dir="rtl">
      ...
      <main className="flex-1 pb-28 md:pb-16">
        {/* ── Hero Banner ── */}
        <div className="relative h-56 md:h-72 w-full overflow-hidden bg-secondary">
          <img
            src={bannerSrc}
              ...
              <Card className="border-border/40 shadow-xl bg-card rounded-2xl overflow-hidden mb-6">
                <CardContent className="p-5 md:p-7">
                  <div className="flex flex-col sm:flex-row gap-5 items-start">
                    {/* Avatar */}
                    <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl border-4 border-background shadow-lg overflow-hidden flex-shrink-0 -mt-14 sm:-mt-10 bg-secondary">
                      <img
                        src={avatarSrc}
                        alt={provider.userName}
                        className="w-full h-full object-cover"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-card rounded-2xl border border-border/40 p-4 flex items-start gap-3 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">متاح طوال أيام الأسبوع</p>
                    </div>
                  </div>
                  <div className="bg-card rounded-2xl border border-border/40 p-4 flex items-start gap-3 shadow-sm">
              ...
              <div className="sticky top-24 space-y-4">
                ...
                <Card className="border-border/40 shadow-xl bg-card rounded-2xl overflow-hidden">
                  ...
                  <CardContent className="p-5">
                    <h3 className="font-bold text-lg mb-4">تواصل مع مقدم الخدمة</h3>
                    <div className="space-y-3">
                      {showPhone ? (
                      ...
                      <Button
                        ...
                        onClick={() => { trackInteraction("whatsapp"); window.open(`https://wa.me/${provider.whatsapp ?? provider.phone ?? ""}`, "_blank"); }}
                      >
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 p-3 bg-background/95 backdrop-blur-md border-t border-border/50 z-50 flex gap-2 shadow-lg">
        <Button
          className="flex-1 h-11 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold border-none shadow-md text-sm"
          onClick={() => { trackInteraction("whatsapp"); window.open(`https://wa.me/${provider.whatsapp ?? provider.phone ?? ""}`, "_blank"); }}
        >
        ...
        <Button
          variant="outline"
          className="flex-1 h-11 rounded-xl border-primary text-primary font-bold hover:bg-primary/10 text-sm"
          onClick={() => { if (!showPhone) trackInteraction("phone"); setShowPhone(!showPhone); }}
        >
          <Phone className="w-4 h-4 ml-1.5" />
          {showPhone ? (provider.phone ?? "اتصال") : "اتصال"}
        </Button>
        <Button
          variant="outline"
          size="icon"
