...
import { api, type Provider, type Category } from "@/lib/api";
...
export default function SearchPage() {
  ...
  const { data: providers, loading } = useApi(
    ...
    [searchInput, selectedCategory, selectedCityName]
  );
  ...
  const sortedProviders = useMemo(() => {
    if (!userLocation) return displayProviders;
    return [...displayProviders].sort((a, b) => {
      const coordsA = getProviderCoords(a);
      const coordsB = getProviderCoords(b);
      const distA = coordsA ? haversine(userLocation.lat, userLocation.lng, coordsA[0], coordsA[1]) : 9999;
      const distB = coordsB ? haversine(userLocation.lat, userLocation.lng, coordsB[0], coordsB[1]) : 9999;
      return distA - distB;
    });
  }, [displayProviders, userLocation]);
    ...
    <div className="space-y-8">
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-lg">المدينة والمنطقة</h3>
        ...
        {selectedAreaNames.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {selectedAreaNames.map(name => (
              <Badge key={name} variant="secondary" className="text-xs bg-teal-100 text-teal-700 border-teal-200">
                {name}
              </Badge>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {[5, 4, 3].map(rating => (
            <div key={rating} className="flex items-center space-x-3 space-x-reverse">
              <Checkbox id={`rating-${rating}`} defaultChecked={rating === 4} className="border-border/60 data-[state=checked]:bg-accent data-[state=checked]:border-accent" />
      ...
      <Button variant="outline" className="w-full h-12 border-destructive/20 text-destructive hover:bg-destructive/5 hover:text-destructive transition-colors"
        onClick={() => { setSelectedCategory("all"); setSelectedCityId(null); setSelectedCityName(null); setSelectedAreaIds([]); setSelectedAreaNames([]); setSearchInput(""); setUserLocation(null); }}>
        مسح جميع الفلاتر
      </Button>
    </div>
  );
      ...
      <div className="sticky top-16 z-40 w-full border-b bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 py-4 shadow-sm">
        <div className="container mx-auto px-4">
          <div className="bg-card p-2 rounded-2xl shadow-sm border border-border/60 flex gap-2 transition-shadow focus-within:shadow-md duration-300">
            ...
            <Button className="h-12 px-8 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-bold transition-transform hover:scale-[1.02]">
              بحث
            </Button>
          </div>
        </div>
          ...
          <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  ...
                  {selectedCityName && (
                    <Badge variant="outline" className="bg-background flex items-center gap-1.5 px-3 py-1.5 border-border/60 font-medium text-muted-foreground">
                      {selectedCityName}
                      <X className="w-3.5 h-3.5 cursor-pointer hover:text-destructive" onClick={() => { setSelectedCityId(null); setSelectedCityName(null); setSelectedAreaIds([]); setSelectedAreaNames([]); }} />
                    </Badge>
                  )}
                  {selectedAreaNames.map(name => (
                    <Badge key={name} variant="outline" className="bg-teal-50 text-teal-700 border-teal-200 flex items-center gap-1.5 px-3 py-1.5 font-medium">
                      {name}
                      <X className="w-3.5 h-3.5 cursor-pointer hover:text-red-500" onClick={() => {
                        const newIds = selectedAreaIds.filter((_, i) => selectedAreaNames[i] !== name);
                        const newNames = selectedAreaNames.filter(n => n !== name);
                        setSelectedAreaIds(newIds);
                        setSelectedAreaNames(newNames);
                      }} />
                    </Badge>
                  ))}
              ...
              <div className={`grid gap-6 ${viewMode === "grid" ? "grid-cols-1 sm:grid-cols-2 xl:grid-cols-3" : "grid-cols-1"}`}>
                {sortedProviders.map((provider, idx) => {
                  ...
                  return (
                    <Card
                      key={`${provider.id}-${idx}`}
                      className={`overflow-hidden group border-border/50 hover:shadow-xl hover:border-primary/20 transition-all duration-300 cursor-pointer relative ${viewMode === "list" ? "flex flex-row h-44" : "flex flex-col"} ${isSponsored ? "bg-amber-50/30 border-amber-200" : ""}`}
                      onClick={() => setLocation(`/provider/${provider.id}`)}
                    >
                      <div className={`relative overflow-hidden bg-muted ${viewMode === "list" ? "w-44 shrink-0" : "h-48"}`}>
                        <img
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                          <Button
                            ...
                            onClick={(e) => { e.stopPropagation(); setLocation("/provider/register"); }}
                          >
                      ...
                      <CardContent className={`p-4 flex flex-col justify-between ${viewMode === "list" ? "flex-1 py-4" : ""}`}>
                        <div>
                          <div className="flex justify-between items-start mb-1.5">
                            <h3 className="font-bold text-base leading-tight group-hover:text-primary transition-colors line-clamp-1">{provider.userName}</h3>
                        ...
                        {viewMode === "list" && (
                          <div className="mt-3 flex items-center justify-end">
                            <Button size="sm" className="bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground shadow-none rounded-xl text-xs">
                              عرض التفاصيل
                            </Button>
                          </div>
                        )}
            ...
            {(providers?.length ?? 0) > 0 && (
              <div className="mt-12 flex justify-center">
                <Button variant="outline" size="lg" className="rounded-full px-12 border-border/60 hover:bg-secondary h-12 font-medium">
                  عرض المزيد
                </Button>
              </div>
            )}
