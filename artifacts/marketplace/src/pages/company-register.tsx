...
function getPasswordStrength(pw: string): { score: number; label: string; color: string } {
  ...
  if (score === 4) return { score, label: "قوية", color: "bg-teal-500" };
  return { score, label: "قوية جداً", color: "bg-emerald-500" };
}

function validateEgyptianPhone(ph: string): boolean {
  return /^(010|011|012|015)\d{8}$/.test(ph.replace(/\s/g, ""));
}

function DragDropZone({
  label, hint, preview, onFile, onClear, aspectRatio = "aspect-[3/1]",
}: {
  label: string;
  hint: string;
  preview: string;
  ...
  return (
    <div
      className={`relative ${aspectRatio} w-full rounded-2xl border-2 border-dashed transition-all duration-200 overflow-hidden group cursor-pointer
...
export default function CompanyRegisterPage() {
  ...
  const validateStep1 = () => {
    ...
    if (form.password.length < 8) e.password = "يجب أن تكون كلمة المرور 8 أحرف على الأقل";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (logoFileRef.current) {
        try { const r = await api.upload.avatar(logoFileRef.current); logoUrl = r.url; } catch {}
      }
      if (bannerFileRef.current) {
        try { const r = await api.upload.banner(bannerFileRef.current); bannerUrl = r.url; } catch {}
      }
        ...
        <div className="max-w-md w-full text-center space-y-6 animate-in zoom-in-95 duration-500">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-lg leading-relaxed">
              مرحباً بك في عقارات بنها. يمكنك الآن نشر عقاراتك والوصول إلى آلاف العملاء.
            </p>
          </div>
          ...
          {[1, 2].map((s, i) => (
            <div key={s} className="flex items-center gap-3 flex-1">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 transition-all duration-300
                ${step > s ? "bg-primary text-white" : step === s ? "bg-primary text-white shadow-lg shadow-primary/30 scale-110" : "bg-zinc-200 text-zinc-500"}`}>
                {step > s ? <Check className="w-4 h-4" /> : s}
              </div>
              <span className={`text-sm font-semibold hidden sm:block transition-colors ${step === s ? "text-primary" : step > s ? "text-zinc-600" : "text-zinc-400"}`}>
                {s === 1 ? "معلومات الحساب" : "بيانات الشركة"}
              </span>
              {i < 1 && <div className={`flex-1 h-0.5 rounded-full transition-all duration-500 ${step > s ? "bg-primary" : "bg-zinc-200"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-zinc-100 overflow-hidden">
          ...
          {step === 1 && (
            <div className="p-6 sm:p-8 space-y-5 animate-in fade-in slide-in-from-right-4 duration-400">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="name" className="text-sm font-semibold text-zinc-700">الاسم الكامل *</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`h-12 pr-10 rounded-xl ${errors.name ? "border-red-400 focus:ring-red-400" : ""}`}
                  />
                </div>
              ...
              {/* Email */}
              <div className="space-y-1.5">
                <Label htmlFor="email" className="text-sm font-semibold text-zinc-700">البريد الإلكتروني *</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`h-12 pr-10 rounded-xl text-right ${errors.email ? "border-red-400 focus:ring-red-400" : ""}`}
                  />
                </div>
              ...
              {/* Phone */}
              <div className="space-y-1.5">
                <Label htmlFor="phone" className="text-sm font-semibold text-zinc-700">رقم الهاتف المصري *</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className={`h-12 pr-10 pl-16 rounded-xl font-mono text-right tracking-wider ${errors.phone ? "border-red-400" : form.phone.length === 11 && validateEgyptianPhone(form.phone) ? "border-emerald-400" : ""}`}
                  />
              ...
              {/* Password */}
              <div className="space-y-1.5">
                <Label htmlFor="password" className="text-sm font-semibold text-zinc-700">كلمة المرور *</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    dir="ltr"
                    value={form.password}
                    onChange={e => { set({ password: e.target.value }); setErrors(prev => ({ ...prev, password: "" })); }}
                    placeholder="••••••••"
                    className={`h-12 pr-10 pl-10 rounded-xl ${errors.password ? "border-red-400" : ""}`}
                  />
              ...
              {/* Confirm Password */}
              <div className="space-y-1.5">
                <Label htmlFor="confirm" className="text-sm font-semibold text-zinc-700">تأكيد كلمة المرور *</Label>
                <div className="relative">
                  <Lock className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
                  <Input
                    id="confirm"
                    ...
                    onChange={e => { set({ confirmPassword: e.target.value }); setErrors(prev => ({ ...prev, confirmPassword: "" })); }}
                    placeholder="••••••••"
          ...
          {step === 2 && (
            <div className="p-6 sm:p-8 space-y-6 animate-in fade-in slide-in-from-right-4 duration-400">
              ...
              {/* Company Name */}
              <div className="space-y-1.5">
                <Label htmlFor="companyName" className="text-sm font-semibold text-zinc-700">اسم الشركة العقارية *</Label>
                <div className="relative">
                  <Building2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
                  <Input
                    id="companyName"
                    value={form.companyName}
                    onChange={e => { set({ companyName: e.target.value }); setErrors(prev => ({ ...prev, companyName: "" })); }}
              ...
              <div className="space-y-1.5">
                ...
                <DragDropZone
                  label="اسحب وأفلت صورة البانر"
                  hint="JPG, PNG — حجم موصى به 1200×400"
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="flex-1 bg-zinc-50 rounded-2xl p-4 border border-zinc-100">
                    ...
                    <ul className="text-xs text-zinc-400 space-y-1">
                      <li>• مربع الشكل 1:1</li>
                      <li>• خلفية شفافة (PNG)</li>
                      <li>• جودة عالية 500px+</li>
                      <li>• لا يزيد عن 5MB</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              {/* Bio */}
              <div className="space-y-1.5">
                <Label htmlFor="bio" className="text-sm font-semibold text-zinc-700">
                  نبذة تعريفية
                  <span className="text-zinc-400 font-normal text-xs mr-1">(اختياري)</span>
                </Label>
                <Textarea
                  id="bio"
                  value={form.bio}
              ...
              <div className="space-y-3 bg-zinc-50 rounded-2xl p-4 border border-zinc-100">
                <div className="flex items-start gap-3">
                  <Checkbox
                    ...
                    onCheckedChange={v => { set({ acceptPrivacy: !!v }); setErrors(prev => ({ ...prev, acceptPrivacy: "" })); }}
                    className="mt-0.5"
                  />
                ...
                <div className="flex items-start gap-3">
                  <Checkbox
                    ...
                    onCheckedChange={v => { set({ acceptTerms: !!v }); setErrors(prev => ({ ...prev, acceptTerms: "" })); }}
                    className="mt-0.5"
                  />
