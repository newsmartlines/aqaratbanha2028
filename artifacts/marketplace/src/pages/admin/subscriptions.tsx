...
function StatusBadge({ isActive, isPastDue, status }: { isActive: boolean; isPastDue: boolean; status: string }) {
  ...
  if (isPastDue || status === "expired") return (
  ...
  if (status === "replaced") return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-orange-50 text-orange-600 border border-orange-200">
      <RefreshCw className="w-3 h-3" /> مُستبدَل
    </span>
  );
...
export default function AdminSubscriptions() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      const matchTab =
        ...
        tab === "expired" ? (r.isPastDue || r.status === "expired" || r.status === "replaced") :
        r.status === "cancelled";
      const matchType =
        typeFilter === "all" ? true :
  ...
  const TABS = [
    ...
    { key: "expired", label: "منتهية / مستبدلة", count: rows.filter(r => r.isPastDue || r.status === "expired" || r.status === "replaced").length },
