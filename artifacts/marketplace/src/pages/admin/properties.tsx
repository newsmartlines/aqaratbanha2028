...
export default function AdminProperties() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      // limit=500 caps the server query; ensureArray extracts .data from the paginated response
      const data = await api.properties.list({ status: "all", limit: 500 });
  ...
  const FormFields = ({ f, setF }: { f: typeof emptyForm; setF: (v: typeof emptyForm) => void }) => (
    <div className="grid gap-4 py-2">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="grid gap-1.5">
          <Label>{f.listingType === "rent" ? "الإيجار (ج.م)" : "السعر (ج.م)"}</Label>
