  ...
  const ListCard = ({ p, idx }: { p: PropertyResult; idx: number }) => {
    ...
    return (
      ...
      <motion.article
        ...
        style={getPromotionCardStyle(p.activePromotion ?? null)}
      ...
      >
        ...
        >
          ...
          {/* Promotion / Featured / Urgent badge — top-right of image */}
          {(p.activePromotion || p.featured || p.urgent) && (
            <div className="absolute top-2 right-2 z-20 pointer-events-none">
              <PromotionBadge
                promotion={
                  p.activePromotion ??
                  (p.featured ? { type: "featured_homepage", boostScore: 500, expiresAt: null } :
                   p.urgent   ? { type: "urgent_badge",      boostScore: 50,  expiresAt: null } : null)
                }
              />
            </div>
          )}
    ...
    return (
      ...
      >
        <PropertyImageGallery images={imgs} alt={p.title} fallback={DEFAULT_IMG} className="h-48">
          ...
          {/* Promotion / Featured / Urgent badge */}
          {(p.activePromotion || p.featured || p.urgent) && (
