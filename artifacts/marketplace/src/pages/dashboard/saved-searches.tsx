...
import {
  Bell, BellOff, Trash2, Loader2, Search, Plus,
} from "lucide-react";
...
function SearchCard({ s, onDelete }: { s: SavedSearch; onDelete: (id: number) => void }) {
  ...
  return (
    <div className="bg-white dark:bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col gap-4">
      ...
      {hasFilters ? (
        ...
        <p className="text-xs text-muted-foreground italic">جميع العقارات (بدون فلاتر محددة)</p>
      )}

      <div className="flex items-center justify-between border-t border-border/50 pt-3 mt-auto">
        <div className="flex items-center gap-3">
          <span className={`inline-flex items-center gap-1 text-xs font-medium ${s.notifyEmail ? "text-green-600" : "text-muted-foreground"}`}>
            {s.notifyEmail
              ? <><Bell className="w-3 h-3" /> إشعار بريد</>
              : <><BellOff className="w-3 h-3" /> بدون بريد</>
            }
          </span>
          <span className={`inline-flex items-center gap-1 text-xs font-medium ${s.notifyApp ? "text-green-600" : "text-muted-foreground"}`}>
            {s.notifyApp
      ...
      {s.email && (
        <p className="text-xs text-muted-foreground -mt-2">
          الإشعارات تُرسل إلى: <span className="text-foreground font-medium">{s.email}</span>
        </p>
      )}
    </div>
  );
    ...
    <DashboardLayout>
      <div className="p-6 max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">تنبيهات البحث المحفوظة</h1>
        ...
        <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/40 rounded-2xl p-4 flex items-start gap-3">
          <div className="w-9 h-9 rounded-xl bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center shrink-0 mt-0.5">
            <Bell className="w-4 h-4 text-indigo-600" />
          </div>
          <div>
            <h3 className="font-semibold text-indigo-900 dark:text-indigo-200 text-sm mb-1">كيف تعمل التنبيهات؟</h3>
            <p className="text-indigo-700/80 dark:text-indigo-300/80 text-xs leading-relaxed">
              عند إضافة أي عقار جديد يطابق معايير بحثك المحفوظ، سيصلك إشعار داخلي فوري وبريد إلكتروني.
              يمكنك إنشاء تنبيه جديد من صفحة{" "}
              <Link href="/properties" className="font-bold underline">العقارات</Link>{" "}
              بالضغط على زر «حفظ البحث».
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-secondary/20 rounded-2xl border border-border/50">
            ...
            <h3 className="text-xl font-bold text-foreground mb-2">لا توجد تنبيهات بعد</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {searches.map((s) => (
              <SearchCard key={s.id} s={s} onDelete={(id) => deleteMut.mutate(id)} />
            ))}
          </div>
        )}
