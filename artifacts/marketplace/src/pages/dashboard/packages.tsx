...
export default function PackagesPage() {
  ...
  // A user has used the free plan if:
  // 1. Their current active sub is the free plan, OR
  // 2. Any entry in their history was a free plan (price = 0 or null)
  const hasUsedFreePlan = Boolean(
    (sub && (sub.packagePrice == null || parseFloat(String(sub.packagePrice)) === 0)) ||
    history.some(h => h.planPrice == null || parseFloat(String(h.planPrice)) === 0)
  );
  ...
  return (
    <DashboardLayout>
      <div className="p-6 max-w-6xl mx-auto space-y-8" dir="rtl">
        ...
        {/* ── Paid plan: payment gateway selection dialog ───────────────────────── */}
        <Dialog open={payModalOpen} onOpenChange={setPayModalOpen}>
          <DialogContent dir="rtl" className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-lg font-bold">اختر طريقة الدفع</DialogTitle>
              {selectedPlan && (
                <DialogDescription>
                  باقة <strong>{selectedPlan.nameAr ?? selectedPlan.name}</strong> — {parseFloat(String(selectedPlan.price)).toLocaleString("ar-EG")} {selectedPlan.currency ?? "ج.م"} / {selectedPlan.durationDays} يوم
                </DialogDescription>
              )}
            </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-3 my-2">
              {(
                [
                  { id: "vodafone_cash",  label: "فودافون كاش",  icon: <Smartphone className="w-5 h-5" />,  color: "border-red-400 bg-red-50 text-red-700" },
                  { id: "fawry",          label: "فوري",          icon: <Zap className="w-5 h-5" />,          color: "border-orange-400 bg-orange-50 text-orange-700" },
                  { id: "instapay",       label: "انستاباي",      icon: <CreditCard className="w-5 h-5" />,   color: "border-blue-400 bg-blue-50 text-blue-700" },
                  { id: "bank_transfer",  label: "تحويل بنكي",   icon: <Building2 className="w-5 h-5" />,    color: "border-green-400 bg-green-50 text-green-700" },
                ] as const
              ).map(g => (
                <button
                  key={g.id}
                  type="button"
                  onClick={() => setPayGateway(g.id)}
                  className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all font-semibold text-sm ${
                    payGateway === g.id
                      ? g.color + " shadow-md scale-[1.02]"
            ...
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
              ...
              ستُرفع بياناتك وإيصال الدفع في الخطوة التالية
            </div>

            <DialogFooter className="gap-2 sm:gap-0 mt-2">
              <Button variant="outline" onClick={() => setPayModalOpen(false)} className="rounded-xl">إلغاء</Button>
              <Button
