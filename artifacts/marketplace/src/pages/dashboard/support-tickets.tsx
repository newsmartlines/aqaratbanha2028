...
import { Inbox, MessageCircleReply, Plus, Search, Ticket, Timer, Loader2, AlertCircle, HeadphonesIcon } from "lucide-react";
...
export default function SupportTicketsPage() {
  ...
  const stats = useMemo(() => ({
    ...
    open: tickets.filter((x) => x.status === "Pending").length,
  ...
  const filtered = useMemo(() => {
    ...
    return tickets.filter((t) => {
      ...
      const searchOk = !q || t.id.toLowerCase().includes(q) || t.subject.toLowerCase().includes(q) || t.category.toLowerCase().includes(q);
    ...
    <DashboardLayout>
      ...
      >
        ...
        <div className="relative mx-auto max-w-6xl space-y-8">
          ...
          {/* Loading state — inline, no separate return */}
          {authLoading && (
