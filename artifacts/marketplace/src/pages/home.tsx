...
import { api, type Category, type Subcategory, type SiteSettings, type Region } from "@/lib/api";
...
function SmallPropertyCard({ p, onClick }: { p: SmallProperty; onClick: () => void }) {
  ...
  const priceStr = priceNum ? priceNum.toLocaleString("en-US") + " جنيه" : "السعر عند التواصل";
...
export default function Home() {
  ...
  const expandedCatObj = (reCategories as Category[] | undefined)?.find(c => c.id === expandedCat);
  const expandedCatIndex = (reCategories as Category[] | undefined)?.findIndex(c => c.id === expandedCat) ?? 0;
          ...
          const MiniCard = ({ property }: { property: any }) => {
            ...
            return (
              ...
              >
                ...
                <div className="p-3.5">
                  <p className="text-right"><span dir="ltr" className="inline text-primary text-base leading-none">{priceStr}</span> <span className="text-[11px] text-primary/70 font-normal">جنيه</span></p>
