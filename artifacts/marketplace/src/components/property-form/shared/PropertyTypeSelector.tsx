...
/* ─── Step header ───────────────────────────────────────────────────────── */
function StepHeader({ n, label, done, active, onReset }: {
    ...
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-2.5">
        <div
          ...
          style={{
            background: done ? "#374151" : active ? "#fff" : "#f9fafb",
            color: done ? "#fff" : active ? "#374151" : "#9ca3af",
            border: done ? "none" : active ? "1.5px solid #374151" : "1.5px solid #e5e7eb",
          }}
        >
          {done ? <CheckCircle2 className="w-3.5 h-3.5" /> : n}
        </div>
        <span className="text-sm font-bold" style={{ color: done || active ? "#111827" : "#9ca3af" }}>
          {label}{active && <span className="text-red-500 mr-0.5"> *</span>}
        </span>
      </div>
      {done && onReset && (
...
function LockOverlay({ msg }: { msg: string }) {
  return (
    <div className="absolute inset-0 rounded-xl flex flex-col items-center justify-center z-10 select-none"
      style={{ background: "rgba(249,250,251,0.90)", backdropFilter: "blur(5px)" }}>
...
function GhostRow({ count = 3 }: { count?: number }) {
  return (
    <div className="grid gap-2 pointer-events-none"
      style={{ gridTemplateColumns: `repeat(${count}, 1fr)`, opacity: 0.1, filter: "blur(3px)" }}>
...
function GhostChips({ count = 5 }: { count?: number }) {
  ...
  return (
    <div className="flex flex-wrap gap-2 pointer-events-none" style={{ opacity: 0.1, filter: "blur(3px)" }}>
      {Array.from({ length: count }).map((_, i) => (
...
function GhostCards() {
  return (
    <div className="grid grid-cols-2 gap-2 pointer-events-none" style={{ opacity: 0.1, filter: "blur(3px)" }}>
...
function Connector({ done }: { done: boolean }) {
  return (
    <div className="flex justify-center py-0.5">
      <div className="w-px rounded-full transition-all duration-500" style={{ height: 16, background: done ? "#6b7280" : "#e5e7eb" }} />
    </div>
  );
}
...
export function PropertyTypeSelector({ v, set, onMainCategoryChange }: PropertyTypeSelectorProps) {
  ...
  const block1 = (
    <div className="rounded-xl border bg-white p-3.5 transition-all duration-300"
      style={{ borderColor: phase >= 1 ? "#d1d5db" : "#e5e7eb" }}>
      <StepHeader n={1} label="نوع العقار" done={phase >= 1} active={phase === 0} onReset={phase >= 1 ? handleGroupReset : undefined} />

      {/* Phase 0: 3 group cards */}
      {phase === 0 && (
          ...
          {PROPERTY_GROUPS.map((group) => {
            ...
            return (
              <button key={group.value} type="button" onClick={() => handleGroupClick(group.value)}
                className="flex flex-col items-center text-center gap-1.5 py-3 px-2 rounded-xl border border-gray-200 bg-white font-semibold transition-all duration-150 hover:border-gray-400 hover:bg-gray-50 focus:outline-none">
          ...
          {(() => {
            ...
            return (
              <button type="button" onClick={handleGroupReset}
                className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-300 bg-gray-50 text-gray-700 font-semibold text-xs transition-colors hover:bg-gray-100">
  ...
  const block2 = (
    <div className="rounded-xl border bg-white p-3.5 transition-all duration-300"
      style={{ borderStyle: isBlock2Locked ? "dashed" : "solid", borderColor: phase >= 2 ? "#d1d5db" : "#e5e7eb" }}>
      <StepHeader n={2} label="نوع الوحدة" done={phase >= 2} active={phase === 1} onReset={phase >= 2 ? handleSubtypeReset : undefined} />
            ...
            {activeGroup.subtypes.map((sub) => {
              ...
              return (
                <button key={sub.value} type="button" onClick={() => handleSubtypeClick(sub.value)}
                  ...
                  {isActive
                    ? <CheckCircle2 className="w-3 h-3 text-gray-700 shrink-0" />
      ...
      {phase >= 2 && (
        <FadeIn show>
          <button type="button" onClick={handleSubtypeReset}
            className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-300 bg-gray-50 text-gray-700 font-semibold text-xs hover:bg-gray-100 transition-colors">
            ...
            {LISTING_OPTS.map((opt) => {
              ...
              return (
                <button key={opt.value} type="button" onClick={() => set("listingType", opt.value)}
                  className="relative flex items-center gap-2.5 p-3 rounded-xl border-2 text-right focus:outline-none transition-all duration-150"
                  style={{
