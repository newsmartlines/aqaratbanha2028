...
router.post("/messages", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Create notification for recipient + fire email
    try {
      ...
      let notifMessage = `أرسل لك ${senderName} رسالة جديدة`;
      ...
      events.onNewMessage(parseInt(receiverId), senderName, content.trim().slice(0, 100)).catch(() => {});
    } catch {}
