...
// ── Admin: reset all areas for a given city back to defaults ──────────────────
router.post("/admin/reset-city-areas", adminOnly, async (req, res) => {
  try {
    const { cityId } = req.body as { cityId?: number };
    if (!cityId) return res.status(400).json({ success: false, error: "cityId required" });
    ...
    // Just report current count — admins can manage areas via the UI
    const existing = await db.select({ id: areasTable.id }).from(areasTable)
      .where(eq(areasTable.cityId, cityId));

    res.json({ success: true, data: { cityId: city.id, cityName: city.nameAr, areaCount: existing.length } });
  } catch (e: any) {
    res.status(500).json({ success: false, error: e?.message ?? "Failed to query city areas" });
  }
});
