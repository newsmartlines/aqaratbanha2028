...
router.post("/admin/properties/:id/bump", async (req, res) => {
  try {
    ...
    // Resolve the owner userId — for provider-owned properties, look up provider owner
    let ownerUserId = property.ownerUserId;
    if (!ownerUserId && property.providerId) {
      const [provRow] = await db
        .select({ userId: providersTable.userId })
        .from(providersTable)
        .where(eq(providersTable.id, property.providerId));
      ownerUserId = provRow?.userId ?? null;
    }
    if (!ownerUserId) ownerUserId = adminUserId;
