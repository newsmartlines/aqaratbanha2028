...
import { sessionsTable, resetTokensTable, verificationTokensTable } from "@workspace/db";
...
async function createVerificationToken(userId: number, email: string): Promise<string> {
  await db.delete(verificationTokensTable).where(eq(verificationTokensTable.userId, userId)).catch(() => {});
  const token = crypto.randomBytes(32).toString("hex");
  const expiresAt = new Date(Date.now() + 60 * 60 * 1000); // 60 min
  await db.insert(verificationTokensTable).values({ token, userId, email, expiresAt });
  return token;
}

async function consumeVerificationToken(token: string): Promise<{ userId: number; email: string } | null> {
  const [row] = await db
    .select()
    .from(verificationTokensTable)
    .where(and(eq(verificationTokensTable.token, token), gt(verificationTokensTable.expiresAt, new Date())));
  if (!row) return null;
  await db.delete(verificationTokensTable).where(eq(verificationTokensTable.token, token)).catch(() => {});
...
router.post("/auth/register", authLimiter, async (req, res) => {
  try {
    ...
    if (phone && !EGYPT_PHONE_REGEX.test(String(phone).trim()))
      return res.status(400).json({ success: false, error: "رقم الهاتف غير صحيح — يجب أن يبدأ بـ 010 أو 011 أو 012 أو 015 ويكون 11 رقماً" });
    ...
    const [user] = await db
      .insert(usersTable)
      .values({ name: String(name).trim(), email: String(email).trim().toLowerCase(), phone: phone ? String(phone).trim() : null, passwordHash, role, regionId: regionIdVal, cityId: cityIdVal })
    ...
    createVerificationToken(user.id, user.email).then(verificationToken => {
      mailer.verifyEmail(user.email, user.name, verificationToken).catch(() => {});
    }).catch(() => {});
...
router.post("/auth/login", authLimiter, async (req, res) => {
  try {
    ...
    const loginDevice = userAgent.includes("Mobile") ? "جهاز محمول" : "جهاز مكتبي";
    mailer.loginAlert(user.email, user.name, loginTime, loginDevice).catch(() => {});
...
router.get("/auth/me", async (req: Request, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      data: { ...user, providerId, providerApproved, staffRole, permissions, isSuperAdmin, emailVerified: user.emailVerified },
    });
  } catch {
...
router.post("/auth/change-password", authLimiter, async (req: Request, res) => {
  const token = req.cookies?.session ?? req.headers.authorization?.replace(/^Bearer\s+/i, "");
  if (!token) return res.status(401).json({ success: false, error: "يجب تسجيل الدخول أولاً" });
  const session = await getSession(token);
  if (!session) return res.status(401).json({ success: false, error: "انتهت الجلسة" });
  ...
  try {
    ...
    // Invalidate all other sessions for security; re-issue a fresh one for the current device
    await deleteAllUserSessions(user.id);
    const newToken = await createSession(session.userId, session.role, session.providerId);
...
router.get("/auth/verify-email", forgotLimiter, async (req, res) => {
  try {
    const token = String(req.query.token ?? "").trim();
    if (!token) return res.status(400).json({ success: false, error: "رمز التحقق مطلوب" });

    const data = await consumeVerificationToken(token);
    if (!data) return res.status(400).json({ success: false, error: "رمز التحقق غير صالح أو منتهي الصلاحية" });

    await db.update(usersTable).set({ emailVerified: true }).where(eq(usersTable.id, data.userId));

    res.json({ success: true, message: "تم التحقق من بريدك الإلكتروني بنجاح" });
  } catch {
    res.status(500).json({ success: false, error: "فشل التحقق من البريد الإلكتروني" });
  }
});

router.post("/auth/resend-verification", forgotLimiter, async (req: Request, res) => {
  const sessionToken = req.cookies?.session ?? req.headers.authorization?.replace(/^Bearer\s+/i, "");
  if (!sessionToken) return res.status(401).json({ success: false, error: "يجب تسجيل الدخول أولاً" });
  const session = await getSession(sessionToken);
  if (!session) return res.status(401).json({ success: false, error: "انتهت الجلسة" });

  try {
    const [user] = await db.select({ id: usersTable.id, name: usersTable.name, email: usersTable.email, emailVerified: usersTable.emailVerified })
      .from(usersTable).where(eq(usersTable.id, session.userId));
    if (!user) return res.status(404).json({ success: false, error: "المستخدم غير موجود" });
    if (user.emailVerified) return res.status(400).json({ success: false, error: "البريد الإلكتروني محقق بالفعل" });

    const verificationToken = await createVerificationToken(user.id, user.email);
    await mailer.verifyEmail(user.email, user.name, verificationToken);

    res.json({ success: true, message: "تم إرسال رابط التحقق إلى بريدك الإلكتروني" });
  } catch {
    res.status(500).json({ success: false, error: "فشل إرسال رابط التحقق" });
  }
});
...
router.post("/auth/google", async (req, res) => {
  ...
  try {
    ...
    if (access_token) {
      // Pass token via Authorization header — never in URL (avoids server log exposure)
      ...
      if (info.email_verified === false) return res.status(400).json({ success: false, error: "بريدك الإلكتروني على Google غير محقق" });
      ...
      if (payload.email_verified === false) return res.status(400).json({ success: false, error: "بريدك الإلكتروني على Google غير محقق" });
