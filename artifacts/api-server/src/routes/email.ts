...
router.use("/admin/email", adminOnly);
...
router.post("/admin/email/smtp/test", async (req, res) => {
  try {
    // Accept live form values from the request body so the user can test
    // without saving first. Fall back to saved DB values for any missing field.
    const saved = await getSmtpConfig();
    const body  = (req.body ?? {}) as Record<string, string | undefined>;

    const cfg = {
      smtpHost:      body.smtpHost      ?? saved.smtpHost,
      smtpPort:      body.smtpPort      ?? saved.smtpPort,
      smtpSecure:    body.smtpSecure    ?? saved.smtpSecure,
      smtpUser:      body.smtpUser      ?? saved.smtpUser,
      smtpPass:      body.smtpPass      ?? saved.smtpPass,
      smtpFromName:  body.smtpFromName  ?? saved.smtpFromName,
      smtpFromEmail: body.smtpFromEmail ?? saved.smtpFromEmail,
    };
    ...
    const transporter = nodemailer.default.createTransport({
      ...
      greetingTimeout:  8000,     // 8s waiting for server greeting
      socketTimeout:    10000,    // 10s socket inactivity
    } as any);

    // Verify connection with a hard 15s deadline
    await Promise.race([
      transporter.verify(),
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("انتهت مهلة الاتصال بخادم SMTP (15 ثانية) — تحقق من الـ Host والـ Port")), 15000)
      ),
    ]);
...
// POST /api/contact — public contact form (sends to site contactEmail)
router.post("/contact", async (req, res) => {
  try {
    ...
    if (!contactEmail)
      return res.status(503).json({ success: false, error: "بريد التواصل غير مُضبوط في إعدادات الموقع" });

    const vars = { siteName, siteUrl, contactEmail, year };
    const safeMsg = (message ?? "").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br/>");
    const html = renderTemplate(
      BASE_HTML(`
        <h2 style="margin:0 0 8px;color:#0f172a;font-size:22px;font-weight:800;">📬 رسالة جديدة عبر نموذج التواصل</h2>
        <p style="margin:0 0 24px;color:#64748b;font-size:14px;">وردت الرسالة التالية من نموذج الاتصال على {{siteName}}.</p>
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;margin-bottom:24px;overflow:hidden;">
          <tr><td style="padding:10px 16px;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;font-weight:600;width:30%;">الاسم</td><td style="padding:10px 16px;color:#0f172a;font-size:13px;border-bottom:1px solid #f1f5f9;">${name}</td></tr>
          <tr><td style="padding:10px 16px;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;font-weight:600;">البريد</td><td style="padding:10px 16px;color:#0f172a;font-size:13px;border-bottom:1px solid #f1f5f9;"><a href="mailto:${email}" style="color:#0d9488;">${email}</a></td></tr>
          ${phone ? `<tr><td style="padding:10px 16px;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;font-weight:600;">الهاتف</td><td style="padding:10px 16px;color:#0f172a;font-size:13px;border-bottom:1px solid #f1f5f9;">${phone}</td></tr>` : ""}
          ${subject ? `<tr><td style="padding:10px 16px;color:#64748b;font-size:13px;border-bottom:1px solid #f1f5f9;font-weight:600;">الموضوع</td><td style="padding:10px 16px;color:#0f172a;font-size:13px;border-bottom:1px solid #f1f5f9;">${subject}</td></tr>` : ""}
          <tr><td style="padding:10px 16px;color:#64748b;font-size:13px;font-weight:600;vertical-align:top;">الرسالة</td><td style="padding:10px 16px;color:#0f172a;font-size:14px;line-height:1.7;">${safeMsg}</td></tr>
        </table>
      `),
      vars,
    );

    const emailSubject = `📬 رسالة جديدة من ${name}${subject ? ` — ${subject}` : ""}`;
    const result = await sendEmail({ to: contactEmail, toName: siteName, subject: emailSubject, html, templateName: "contact-form" });
    res.json({ success: true, message: "تم إرسال رسالتك بنجاح", data: result });
  } catch (e: any) {
    res.status(500).json({ success: false, error: e?.message ?? "فشل إرسال الرسالة" });
  }
});
