...
import { eq, desc, and, sql, gte, sum, isNull, ne, aliasedTable } from "drizzle-orm";
...
router.get("/admin/support-tickets", async (_req, res) => {
  try {
    // Use aliases: providerUser = user resolved via provider FK, directUser = user resolved via direct userId
    const providerUser = aliasedTable(usersTable, "provider_user");
    const directUser = aliasedTable(usersTable, "direct_user");
    ...
    const rows = await db
      .select({
        ...
        // Provider-submitted ticket: name/email come via providers → provider_user
        providerUserName: providerUser.name,
        providerUserEmail: providerUser.email,
        // Direct-user ticket: name/email come via direct_user
      ...
      .leftJoin(providerUser, eq(providersTable.userId, providerUser.id))
      .leftJoin(directUser, eq(supportTicketsTable.userId, directUser.id))
    ...
    const data = rows.map((r) => ({
      ...
      userId: r.userId,
      // Prefer provider-path name/email; fall back to direct user path
      providerName: r.providerUserName ?? r.directUserName ?? null,
      providerEmail: r.providerUserEmail ?? r.directUserEmail ?? null,
      subject: r.subject,
      category: r.category,
      status: r.status,
      message: r.message,
