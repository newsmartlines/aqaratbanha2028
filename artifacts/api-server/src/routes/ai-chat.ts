...
const LOCATIONS = [
  ...
  // مراكز
  "أبو قير", "إيدكو", "برج العرب", "مدينة برج العرب الجديدة", "العلمين الجديدة",
  // عام
  "الإسكندرية", "اسكندرية",
];
...
const AREA_KNOWLEDGE: Record<string, string> = {
  "سيدي بشر":   "**سيدي بشر** من أرقى مناطق الإسكندرية 🌊\n• قريبة من الشاطئ وتتميز بالهدوء والأسرة الراقية\n• خدمات ممتازة: مستشفيات، مدارس، مراكز تجارية\n• الأسعار مرتفعة نسبياً نظراً للموقع المميز",
  "سموحة":      "**سموحة** قلب الإسكندرية الحديثة 🏙️\n• مركز تجاري وسكني متكامل، بها نادي سموحة الشهير\n• خدمات ممتازة وموقع مميز وسط الإسكندرية\n• أسعار مناسبة وسهولة الوصول لجميع أنحاء المدينة",
  "المنتزه":    "**المنتزه** منطقة راقية على البحر المتوسط 🌿\n• بها قصر الملك فاروق وحدائق جميلة\n• هادئة ومناسبة للعائلات، أسعارها مرتفعة\n• من أكثر المناطق طلباً في الإسكندرية",
  "كليوباترا":  "**كليوباترا** منطقة راقية في شرق الإسكندرية 👑\n• بها مستوى معيشي مرتفع وخدمات متكاملة\n• أسعارها مرتفعة نسبياً لكنها تستحق الاستثمار\n• قريبة من وسط المدينة والمحاكم والمستشفيات",
  "سيدي جابر": "**سيدي جابر** موقع محوري في الإسكندرية 🚉\n• بها محطة سيدي جابر وسوق كبير\n• خدمات ممتازة وحركة تجارية نشطة\n• أسعار متوسطة ومناسبة للجميع",
  "العجمي":     "**العجمي** على الساحل الغربي للإسكندرية 🏖️\n• مناطق ساحلية جميلة وهواء بحري منعش\n• أسعارها معقولة مقارنة بشرق الإسكندرية\n• مناسبة للعائلات التي تفضل الهدوء والبحر",
  "لوران":      "**لوران** من أفخم مناطق الإسكندرية 🏆\n• تتميز بالرقي والهدوء وقربها من الكورنيش\n• الأسعار مرتفعة لكنها من أفضل الاستثمارات\n• يقطنها طبقة المثقفين والمتعلمين",
  "المعمورة":   "**المعمورة** منطقة ساحلية مميزة 🌊\n• بها شاطئ المعمورة الشهير وهواء بحري نقي\n• أسعار معقولة مقارنة بالمناطق الوسطى\n• مناسبة لمحبي الهدوء والبحر",
  "برج العرب":  "**برج العرب** مدينة صناعية وسكنية متطورة 🏗️\n• بها المنطقة الصناعية ومطار برج العرب الدولي\n• أسعارها في متناول اليد وبها مشاريع جديدة\n• مناسبة للاستثمار والصناعة والسكن الاقتصادي",
  "أبو قير":    "**أبو قير** منطقة ساحلية تاريخية 🐟\n• مشهورة بالمأكولات البحرية وشاطئها الجميل\n• أسعار معقولة وهواء بحري نقي\n• مناسبة لمن يبحث عن الهدوء بعيداً عن الزحام",
};
...
function mapProperty(p: any) {
  ...
  return {
    ...
    location: [p.district, p.address].filter(Boolean).join("، ") || "الإسكندرية",
...
function buildSuggestions(intent: Intent, resultCount: number): string[] {
  ...
  if (resultCount > 0) {
    if (!intent.listingType) {
      ...
      s.push("في سيدي بشر", "في سموحة", "في كليوباترا");
    ...
    s.push("شقة للبيع في سيدي بشر", "أرض للبيع", "شقة للإيجار", "فيلا للبيع");
  }
...
router.post("/ai-chat", async (req, res) => {
  ...
  try {
    ...
    if (intent.isThankYou) {
      ...
      return res.json({ reply: pick(responses), properties: [], suggestions: ["شقة للبيع في الإسكندرية", "أرض للبيع", "فيلا للبيع"] });
    }
    ...
    if (intent.isGreeting) {
      ...
      const greetings = [
        `${timeGreet} 😊\nأنا **مساعد عقارات الإسكندرية الذكي** — يسعدني أساعدك تلاقي عقارك المثالي!\n\nأخبرني إيه اللي بتدور عليه:\n_مثلاً: "شقة 3 غرف للبيع في سيدي بشر" أو "أرض للبيع بأقل من مليون"_`,
        `${timeGreet}! أهلاً وسهلاً 🏠\nأنا هنا أساعدك في البحث عن العقار المناسب في الإسكندرية.\n\nما الذي تبحث عنه؟`,
      ];
      return res.json({
        reply: pick(greetings), properties: [],
        suggestions: ["شقة للبيع في سيدي بشر", "فيلا للبيع", "أرض للبيع", "شقة للإيجار"],
      });
    }
    ...
    if (intent.isAreaQuery) {
      ...
      return res.json({
        reply: "عن أي منطقة تريد معلومات؟ 🗺️\n\nأغطي معلومات عن:\n**سيدي بشر** — **سموحة** — **كليوباترا** — **المنتزه** — **لوران** — **العجمي** — **برج العرب**",
        properties: [],
        suggestions: ["معلومات عن سيدي بشر", "معلومات عن سموحة", "معلومات عن كليوباترا"],
      });
    }
    ...
    if (intent.isCheapestQuery) {
      ...
      if (props.length === 0) return res.json({ reply: `مش لاقي ${catLabel} حالياً. جرب بحث مختلف؟ 🔍`, properties: [], suggestions: ["شقة للبيع في الإسكندرية", "أرض للبيع"] });
      return res.json({
    ...
    if (properties.length === 0) {
      ...
      suggestions = ["شقة للبيع في الإسكندرية", "أرض للبيع", "شقة للإيجار", "فيلا للبيع"];
    } else {
...
router.get("/ai-autocomplete", async (req, res) => {
  try {
    ...
    const statics = [
      "شقة للبيع في الإسكندرية", "شقة للإيجار في الإسكندرية", "فيلا للبيع", "أرض للبيع في الإسكندرية",
      "شقة 3 غرف للبيع", "شقة 2 غرف للإيجار", "محل تجاري للإيجار", "دوبلكس للبيع",
      "شقة مفروشة للإيجار", "أرض سكنية للبيع", "مكتب للإيجار", "أرخص شقة",
      "أحدث العقارات", "إحصائيات السوق", "معلومات عن سيدي بشر",
...
router.get("/chatbot-trending", async (_req, res) => {
  try {
    ...
    const staticQueries = ["شقة للبيع في الإسكندرية", "شقة للإيجار", "فيلا للبيع", "أرض للبيع", "أرخص العقارات", "أحدث العقارات"];
    ...
    return res.json({
      ...
      whatsapp: settings.chatbotWhatsapp || "", botName: settings.chatbotBotName || "مساعد عقارات الإسكندرية",
