...
router.post("/notifications", async (req, res) => {
  // Creating notifications is an admin-only action; they are normally created
  // server-side by event handlers, not by end-users.
  try {
    const session = await getSessionUser(req);
...
router.patch("/notifications/:id/read", async (req, res) => {
  try {
    ...
    // Fetch the notification to verify ownership before marking read
    ...
    const isOwner = notif.userId === session.userId;
    const isAdminGlobal = notif.userId === null && session.role === "admin";
...
router.delete("/notifications/:id", async (req, res) => {
  try {
    ...
    const id = parseInt(req.params.id);
    // Fetch the notification to verify ownership before deleting
    const [notif] = await db.select().from(notificationsTable).where(eq(notificationsTable.id, id));
    if (!notif) return res.status(404).json({ success: false, error: "Notification not found" });
