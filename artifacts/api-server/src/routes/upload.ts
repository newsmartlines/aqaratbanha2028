...
/**
 * Validates font file magic bytes.
...
function isValidFontFile(filePath: string): boolean {
  ...
  // TTF: starts with 00 01 00 00
  if (b[0] === 0x00 && b[1] === 0x01 && b[2] === 0x00 && b[3] === 0x00) return true;
  // OTF: "OTTO"
  if (b[0] === 0x4F && b[1] === 0x54 && b[2] === 0x54 && b[3] === 0x4F) return true;
  // WOFF: "wOFF"
  if (b[0] === 0x77 && b[1] === 0x4F && b[2] === 0x46 && b[3] === 0x46) return true;
  // WOFF2: "wOF2"
  if (b[0] === 0x77 && b[1] === 0x4F && b[2] === 0x46 && b[3] === 0x32) return true;
  return false;
}
...
router.post("/upload/font", async (req: Request, res: any) => {
  ...
  fontUpload.single("font")(req as any, res as any, (err: any) => {
    ...
    if (!isValidFontFile(file.path)) {
      deleteFile(file.path);
      return res.status(400).json({
        success: false,
        error: "الملف ليس خطاً حقيقياً (TTF/OTF/WOFF/WOFF2) — يرجى التحقق من نوع الملف",
      });
    }
