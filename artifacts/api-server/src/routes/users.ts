...
router.get("/users/:userId/subscriptions-history", async (req, res) => {
  try {
    ...
    // Only the owner or an admin may view subscription history
    const callerId = await sessionUserId(req);
    if (!callerId) return res.status(401).json({ success: false, error: "يجب تسجيل الدخول أولاً" });
    ...
    if (callerRow?.role !== "admin" && callerId !== userId)
      return res.status(403).json({ success: false, error: "غير مصرح لك بالوصول لهذا المورد" });
...
router.post("/users/:userId/subscribe", async (req, res) => {
  try {
    ...
    // Only allow the user themselves or an admin to subscribe on their behalf
    const [callerRow] = await db.select({ role: usersTable.role }).from(usersTable).where(eq(usersTable.id, callerId));
    const callerIsAdmin = callerRow?.role === "admin";
    if (!callerIsAdmin && callerId !== userId)
    ...
    // ── Paid plans must go through the payment flow — never auto-activate ──
    if (requestedPrice > 0 && !callerIsAdmin)
      return res.status(402).json({
        success: false,
        error: "الباقات المدفوعة تتطلب إتمام عملية الدفع أولاً",
        code: "PAYMENT_REQUIRED",
      });

    if (requestedPrice === 0) {
      // Only block if there is an active free subscription
...
router.get("/users/:userId/current-subscription", async (req, res) => {
  try {
    ...
    // Only the owner or an admin may view their subscription
    const callerId = await sessionUserId(req);
    if (!callerId) return res.status(401).json({ success: false, error: "يجب تسجيل الدخول أولاً" });
    const [callerRow] = await db.select({ role: usersTable.role }).from(usersTable).where(eq(usersTable.id, callerId));
    if (callerRow?.role !== "admin" && callerId !== userId)
...
router.post("/payments/subscription-request", async (req, res) => {
  try {
    ...
    // Detect if this user is also a provider — store providerId so provider stats can find this subscription
    const [providerRow] = await db
      .select({ id: providersTable.id })
      .from(providersTable)
      .where(eq(providersTable.userId, userId));
    const linkedProviderId = providerRow?.id ?? null;
    ...
    await db.insert(notificationsTable).values({
      ...
      message: `"${userName}" طلب الاشتراك في باقة ${bp.nameAr ?? bp.name} بقيمة ${bp.price} ج.م — بوابة: ${gateway ?? "غير محدد"}`,
      type: "payment",
      link: "/admin/payments",
    }).catch(() => {});
