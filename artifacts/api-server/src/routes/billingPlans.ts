...
router.use("/admin/billing", adminOnly);
...
router.delete("/admin/billing/plans/:id", async (req, res) => {
  try {
    ...
    // Prevent deletion of a plan that has active subscribers
    const [activeSub] = await db
      .select({ id: subscriptionsTable.id })
      .from(subscriptionsTable)
      .where(and(eq(subscriptionsTable.billingPlanId, planId), eq(subscriptionsTable.status, "active")))
      .limit(1);
    if (activeSub) {
      return res.status(409).json({
        success: false,
        error: "لا يمكن حذف هذه الباقة لأن لديها مشتركين نشطين حالياً",
      });
    }
    await db.delete(billingPlansTable).where(eq(billingPlansTable.id, planId));
