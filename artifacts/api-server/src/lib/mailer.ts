...
// ─── Slug mapping: mailer event type → DB template slug ──────────────────────
// The built-in event type names don't always match the DB template slugs.
// This map ensures DB template overrides are found correctly.
const SLUG_MAP: Record<string, string> = {
  "forgot-password":   "password-reset",
  "package-purchased": "subscription-activated",
  "provider-approved": "account-approved",
  "provider-rejected": "account-rejected",
};

function getDbSlug(eventType: string): string {
  return SLUG_MAP[eventType] ?? eventType;
}
...
type EmailEvent =
  ...
  | { type: "email-verification"; userName: string; verificationUrl: string; expiryMinutes: number }
  | { type: "login-alert"; userName: string; loginTime: string; loginDevice: string; siteUrl: string }
  ...
  | { type: "property-expired"; userName: string; propertyTitle: string; siteUrl: string }
  ...
  | { type: "subscription-expiry-warning"; userName: string; packageName: string; expiryDate: string; daysLeft: number; siteUrl: string }
  | { type: "subscription-expired"; userName: string; packageName: string; siteUrl: string }
  | { type: "provider-approved"; userName: string; siteUrl: string }
  | { type: "provider-rejected"; userName: string; rejectionReason: string; siteUrl: string };
...
function buildEmailContent(event: EmailEvent, cfg: Record<string, string>): { subject: string; html: string } {
  ...
  switch (event.type) {
    ...
    case "email-verification": {
      const html = baseHtml(`
        ${heading("✉️", "تأكيد بريدك الإلكتروني", `مرحباً ${event.userName}، شكراً لتسجيلك! يرجى تأكيد بريدك الإلكتروني لتفعيل حسابك.`)}
        ${alertBox("info", `هذا الرابط صالح لمدة ${event.expiryMinutes} دقيقة فقط.`)}
        ${ctaButton("تأكيد البريد الإلكتروني ✉️", event.verificationUrl)}
        <p style="text-align:center;color:#94a3b8;font-size:13px;margin-top:16px;">إذا لم تقم بالتسجيل، تجاهل هذا البريد.</p>
      `, cfg);
      return { subject: `✉️ تأكيد بريدك الإلكتروني — ${siteName}`, html };
    }

    case "login-alert": {
      const html = baseHtml(`
        ${heading("🔔", "تنبيه: تسجيل دخول جديد", `مرحباً ${event.userName}، تم تسجيل الدخول إلى حسابك.`)}
        ${infoBox([
          { label: "الوقت", value: event.loginTime },
          { label: "الجهاز", value: event.loginDevice },
        ])}
        ${alertBox("warning", "إذا لم تكن أنت من قام بتسجيل الدخول، يرجى تغيير كلمة المرور فوراً وتواصل معنا.")}
        ${ctaButton("تأمين حسابي", `${event.siteUrl}/dashboard/security`)}
      `, cfg);
      return { subject: `🔔 تنبيه: تسجيل دخول جديد — ${siteName}`, html };
    }
    ...
    case "property-expired": {
      const html = baseHtml(`
        ${heading("⏰", "انتهت مدة إعلانك", `مرحباً ${event.userName}، انتهت مدة نشر إعلانك.`)}
        ${infoBox([
          { label: "العقار", value: event.propertyTitle },
          { label: "الحالة", value: "⏰ منتهي الصلاحية" },
        ])}
        ${alertBox("warning", "إعلانك لم يعد ظاهراً في نتائج البحث. يمكنك تجديده بنقرة واحدة من لوحة التحكم.")}
        ${ctaButton("تجديد الإعلان 🔄", `${siteUrl}/user/my-properties`)}
        <p style="text-align:center;color:#94a3b8;font-size:13px;margin-top:8px;">التجديد يعيد إعلانك فوراً إلى نتائج البحث والخريطة.</p>
      `, cfg);
      return { subject: `⏰ انتهت مدة إعلانك — ${event.propertyTitle}`, html };
    }
    ...
    case "subscription-expiry-warning": {
      const html = baseHtml(`
        ${heading("⏰", "باقتك على وشك الانتهاء!", `مرحباً ${event.userName}، باقتك ستنتهي قريباً.`)}
        ${infoBox([
          { label: "الباقة", value: event.packageName },
          { label: "تاريخ الانتهاء", value: event.expiryDate },
          { label: "الوقت المتبقي", value: `${event.daysLeft} ${event.daysLeft === 1 ? "يوم" : "أيام"}` },
        ])}
        ${alertBox("warning", "جدّد باقتك الآن لتستمر في الاستفادة من جميع المزايا بدون انقطاع.")}
        ${ctaButton("تجديد الباقة الآن 🔄", `${siteUrl}/dashboard/packages`)}
      `, cfg);
      return { subject: `⏰ تنبيه: باقة ${event.packageName} ستنتهي خلال ${event.daysLeft} ${event.daysLeft === 1 ? "يوم" : "أيام"} — ${siteName}`, html };
    }

    case "subscription-expired": {
      const html = baseHtml(`
        ${heading("🔴", "انتهت مدة باقتك", `مرحباً ${event.userName}، انتهت مدة باقة ${event.packageName}.`)}
        ${alertBox("warning", "لم تعد تستمتع بمزايا الباقة المدفوعة. جدّد باقتك الآن للعودة إلى الحالة الكاملة.")}
        ${ctaButton("تجديد الباقة الآن 🔄", `${siteUrl}/dashboard/packages`)}
      `, cfg);
      return { subject: `🔴 انتهت باقة ${event.packageName} — ${siteName}`, html };
    }

    case "provider-approved": {
      const html = baseHtml(`
        ${heading("✅", "تم قبول حسابك كشركة عقارية!", `مبروك ${event.userName}! تمت الموافقة على حسابك وأصبح ملفك ظاهراً للعملاء.`)}
        ${alertBox("success", "يمكنك الآن إضافة وإدارة إعلاناتك العقارية والتواصل مع العملاء مباشرة.")}
        ${ctaButton("الذهاب إلى لوحة التحكم →", `${siteUrl}/dashboard`)}
      `, cfg);
      return { subject: `✅ تم قبول حسابك كشركة عقارية — ${siteName}`, html };
    }

    case "provider-rejected": {
      const html = baseHtml(`
        ${heading("❌", "بخصوص طلب تسجيل شركتك", `مرحباً ${event.userName}، بعد مراجعة طلبك لم نتمكن من قبوله حالياً.`)}
        <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;padding:20px 24px;margin-bottom:28px;">
          <p style="margin:0 0 8px;color:#0f172a;font-weight:700;font-size:14px;">📋 سبب الرفض:</p>
          <p style="margin:0;color:#475569;font-size:14px;line-height:1.7;">${event.rejectionReason || "لم يتطابق الطلب مع شروط التسجيل."}</p>
        </div>
        ${alertBox("info", "يمكنك تصحيح البيانات والتقدم مجدداً أو التواصل معنا للاستفسار.")}
      `, cfg);
      return { subject: `❌ بخصوص طلب تسجيل شركتك — ${siteName}`, html };
    }
...
async function sendMail(opts: {
  ...
  try {
    ...
    // Look for a matching DB template override (use SLUG_MAP for correct slug)
    let finalSubject = subject;
    let finalHtml = html;
    try {
      const dbSlug = getDbSlug(opts.event.type);
      const [tpl] = await db.select()
        .from(emailTemplatesTable)
        .where(eq(emailTemplatesTable.slug, dbSlug));
...
export const mailer = {
  ...
  async verifyEmail(to: string, name: string, verificationToken: string) {
    const cfg = await getSmtpConfig();
    const siteUrl = cfg.siteUrl || "";
    const verificationUrl = siteUrl
      ? `${siteUrl}/verify-email?token=${verificationToken}`
      : `/verify-email?token=${verificationToken}`;
    return sendMail({ to, toName: name, event: { type: "email-verification", userName: name, verificationUrl, expiryMinutes: 60 } });
  },

  /** Send login alert email */
  async loginAlert(to: string, name: string, loginTime: string, loginDevice: string) {
    const cfg = await getSmtpConfig();
    return sendMail({ to, toName: name, event: { type: "login-alert", userName: name, loginTime, loginDevice, siteUrl: cfg.siteUrl || "" } });
  },
  ...
  /** Notify owner that property listing has expired */
  async propertyExpired(to: string, name: string, propertyTitle: string) {
    const cfg = await getSmtpConfig();
    return sendMail({ to, toName: name, event: { type: "property-expired", userName: name, propertyTitle, siteUrl: cfg.siteUrl || "" } });
  },
  ...
  /** Warn user that their subscription expires soon */
  async subscriptionExpiryWarning(to: string, name: string, packageName: string, expiryDate: string, daysLeft: number) {
    const cfg = await getSmtpConfig();
    return sendMail({ to, toName: name, event: { type: "subscription-expiry-warning", userName: name, packageName, expiryDate, daysLeft, siteUrl: cfg.siteUrl || "" } });
  },

  /** Notify user that their subscription has expired */
  async subscriptionExpired(to: string, name: string, packageName: string) {
    const cfg = await getSmtpConfig();
    return sendMail({ to, toName: name, event: { type: "subscription-expired", userName: name, packageName, siteUrl: cfg.siteUrl || "" } });
  },

  /** Notify provider that their account was approved */
  async providerApproved(to: string, name: string) {
    const cfg = await getSmtpConfig();
    return sendMail({ to, toName: name, event: { type: "provider-approved", userName: name, siteUrl: cfg.siteUrl || "" } });
  },

  /** Notify provider that their account was rejected */
  async providerRejected(to: string, name: string, rejectionReason: string) {
    const cfg = await getSmtpConfig();
    return sendMail({ to, toName: name, event: { type: "provider-rejected", userName: name, rejectionReason, siteUrl: cfg.siteUrl || "" } });
  },
};
