...
export async function expirePromotions(): Promise<number> {
  ...
  for (const p of expired) {
    ...
    // featured_homepage or legacy "featured" → unset property.featured if no other active featured promo
    if (p.type === "featured" || p.type === "featured_homepage") {
      const [stillActive] = await db
        ...
        .where(
          and(
            ...
            inArray(propertyPromotionsTable.type, ["featured", "featured_homepage"]),
            eq(propertyPromotionsTable.isActive, true)
          )
        )
        .limit(1);
    ...
    // urgent_badge → unset property.urgent if no other active urgent promo
    if (p.type === "urgent_badge") {
      const [stillActiveUrgent] = await db
        .select({ id: propertyPromotionsTable.id })
        .from(propertyPromotionsTable)
        .where(
          and(
      ...
      if (!stillActiveUrgent) {
        await db.update(propertiesTable).set({ urgent: false }).where(eq(propertiesTable.id, p.propertyId));
      }
    }
  }
 ...
 *
 * Formula:
 *  premium_listing   → +1000 (highest tier)
 *  spotlight         → +800
 *  featured_homepage → +500
 *  featured_category → +350
 *  featured (legacy) → +500
 *  bump_up / bump    → +200 (decayed by time remaining)
 *  urgent_badge      → +50
 *  featured flag     → +500 (property column)
 *  urgent flag       → +50  (property column)
...
export function computeRankScore(
  ...
  const pType = activePromotion?.type ?? "";

  if (pType === "premium_listing") {
    score += activePromotion!.boostScore ?? 1000;
  } else if (pType === "spotlight") {
    score += activePromotion!.boostScore ?? 800;
  } else if (pType === "featured_homepage" || pType === "featured") {
    score += activePromotion!.boostScore ?? 500;
  } else if (pType === "featured_category") {
    score += activePromotion!.boostScore ?? 350;
  } else if (pType === "bump_up" || pType === "bump") {
    ...
    if (activePromotion!.expiresAt) {
      ...
      const decay = Math.max(0, (msExpiry - Date.now()) / msDuration);
      bumpScore = Math.round(bumpScore * Math.min(1, decay));
    }
    score += bumpScore;
  } else if (pType === "urgent_badge") {
    score += activePromotion!.boostScore ?? 50;
  }
