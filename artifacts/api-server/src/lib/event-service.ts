...
import { notificationsTable, usersTable, providersTable } from "@workspace/db";
...
/**
 * Resolves the notification target userId for a property.
 * For user-owned properties: ownerUserId directly.
 * For provider-owned properties: look up the provider's owner userId.
 */
async function resolvePropertyOwnerUserId(property: {
  ownerUserId?: number | null;
  providerId?: number | null;
}): Promise<number | null> {
  if (property.ownerUserId) return property.ownerUserId;
  if (property.providerId) {
    try {
      const [prov] = await db
        .select({ userId: providersTable.userId })
        .from(providersTable)
        .where(eq(providersTable.id, property.providerId));
      return prov?.userId ?? null;
    } catch { return null; }
  }
  return null;
}
...
export const events = {
  ...
  async onPropertySubmitted(property: {
    ...
    const ownerId = await resolvePropertyOwnerUserId(property);
    if (ownerId) {
